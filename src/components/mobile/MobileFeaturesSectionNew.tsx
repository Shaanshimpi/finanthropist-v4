...
import { initMobileFeaturesTimeline } from '../animations/mobile/mobileFeaturesTimeline'

type MobileFeaturesSectionProps = {
  scheduleAnimation?: MobileScheduleAnimation
}

export const MobileFeaturesSectionNew: React.FC<MobileFeaturesSectionProps> = ({ scheduleAnimation }) => {
  ...
  useEffect(() => {
    ...
    const cards = cardsContainerRef.current?.querySelectorAll('.mobile-feature-card')
    ...
    const runAnimation = () => {
      animationCleanup = initMobileFeaturesTimeline({
        section: sectionEl,
        cards: cards,
      })
    }
  ...
  }, [scheduleAnimation, features.items.length])

  return (
    <section
      ref={sectionRef}
      className="mobile-features-section relative px-6"
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 pb-8 h-full flex flex-col">
        ...
        >
          {features.items.map((feature, index) => (
            <div
              key={`feature-${index}`}
              className="mobile-feature-card flex flex-col justify-between rounded-3xl border border-white/15 bg-slate-900/80"
              style={{
                position: 'absolute',
                inset: 0,
                padding: '1.5rem',
...
export default MobileFeaturesSectionNew
